...
- добавил `ru.javawebinar.topjava.UserTestData.USER_WITH_MEALS_MATCHER` (проверки пользователя сразу с едой) и константу
  id `NOT_FOUND`
...
- Тесты:
  ...
  - **Сделал типизированный `TestMatchers<T>` для проверки результатов тестов. В
    классах `ru.javawebinar.topjava.UserTestData` и `MealTestData` создаю его инстансы с заданным типом и методикой
    сравнения.**
